...
// Not destructured: these are getters that resolve the harness lazily and throw
// when there is none. See lib/paths.js.
...
function canonicalAgents() {
  const subagentsDir = paths.subagentsDir;
...
// Remove symlinks that point into OUR canonical dir but whose canonical entry no
// longer exists (a skill/agent deleted from the harness). Only ever touches
// symlinks whose readlink target is inside `canonicalRoot` — a project-owned real
// dir or a symlink into some other tool is never ours to remove. The target being
// broken is expected (the canonical dir is gone), so don't require it to exist.
function pruneStale(containerDir, canonicalRoot, keep) {
  const pruned = [];
  if (!fs.existsSync(containerDir)) return pruned;
  const rootPrefix = canonicalRoot + path.sep;
  for (const entry of fs.readdirSync(containerDir)) {
    if (keep.has(entry)) continue;
    const linkPath = path.join(containerDir, entry);
    let st = null;
    try { st = fs.lstatSync(linkPath); } catch { continue; }
    if (!st.isSymbolicLink()) continue;
    let target;
    try { target = path.resolve(containerDir, fs.readlinkSync(linkPath)); } catch { continue; }
...
function linkEnv(envPath) {
  ...
  const pruned = [
    ...pruneStale(skillsTarget, paths.skillsDir, skillNames),
    ...pruneStale(agentsTarget, paths.subagentsDir, agentNames),
  ];
  return { linked, agents, ejected, pruned };
}
...
async function cli(args) {
  ...
  const { linked, agents, ejected, pruned } = linkEnv(target);
  ...
  if (pruned.length) console.log(`pruned stale links (canonical source removed): ${pruned.join(', ')}`);
}
